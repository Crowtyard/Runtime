...
事件表 immutable：Repository 层只允许 INSERT（17 节）。"""
...
class WorldRuntime(Base):
    ...
    runtime_status: Mapped[str] = mapped_column(String(24), nullable=False, default="NOT_ACTIVATED")
    last_simulated_real_time: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    current_blessed_time: Mapped[int | None] = mapped_column(Integer, nullable=True)  # 福地年整数刻度 / NULL
...
class TimeRatioHistory(Base):
    ...
    world_id: Mapped[str] = mapped_column(String(64), ForeignKey("world_runtime.world_id"), nullable=False)
    real_effective_from: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    blessed_effective_from: Mapped[str | None] = mapped_column(String(32), nullable=True)  # NULL=尚未开始计
...
class SimulationRun(Base):
    ...
    target_blessed_time: Mapped[int | None] = mapped_column(Integer, nullable=True)
    committed_until: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class SimulationCheckpoint(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    world_id: Mapped[str] = mapped_column(String(64), ForeignKey("world_runtime.world_id"), nullable=False)
    checkpoint_blessed_time: Mapped[int | None] = mapped_column(Integer, nullable=True)
...
class WorldEvent(Base):
    """不可变事件（17 节）：禁止 UPDATE/DELETE；纠错=新事件（supersedes/reverses）。"""
    ...
    blessed_time: Mapped[int | None] = mapped_column(Integer, nullable=True)
    real_time: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
...
class NarrativeRecord(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    world_id: Mapped[str] = mapped_column(String(64), ForeignKey("world_runtime.world_id"), nullable=False)
    source_event_ids: Mapped[dict] = mapped_column(JSON, nullable=False, default=list)
