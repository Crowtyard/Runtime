...
from services.db_lifecycle import (migrate_database,  # noqa: E402
                                   seed_database)


def main() -> None:
    settings = Settings()
    migrate_database(settings.database_url, project_root=PROJECT_ROOT)
    engine = create_db_engine(settings.database_url)
    session_factory = make_session_factory(engine)
    seed_database(engine, session_factory, bible_dir=settings.bible_dir)
