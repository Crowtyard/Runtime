...
from XiaoguangBlessedLandRuntime.services.simulation.state_hash import (
    WORLD_STATE_HASH_SCHEMA_VERSION_V3, world_state_hash_v3)
...
def test_m2b_baseline_120y_artifact(tmp_path):
    ...
    artifact = _build_m2b_artifact(env, rep, wall)
    golden = load_artifact(BASELINE_PATH)
    assert_deterministic_equal(
        golden, artifact, label="m2b_resource_economy_miniworld_120y_v1",
    ...
    pm = golden["population_metrics"]
    assert rep2.population == {"start": pm["start"], "end": pm["end"],
                               "births": pm["births"], "deaths": pm["deaths"],
                               "migrations": pm["emigration"]}
    assert rep2.resource["initial_reserve"] == golden["resource_metrics"][
        "initial_reserve"]
    assert rep2.resource["final_reserve"] == golden["resource_metrics"][
        "final_reserve"]
    assert rep2.resource["total_extracted"] == golden["resource_metrics"][
        "total_extracted"]
    em = golden["economy_metrics"]
...
def _write_m2b_baseline(env, rep, wall_seconds: float) -> None:
    ...
    dump_artifact(_build_m2b_artifact(env, rep, wall_seconds), BASELINE_PATH)


def _build_m2b_artifact(env, rep, wall_seconds: float) -> dict:
    with env["factory"]() as s:
    ...
    artifact = {
        ...
        "world_state_hash_schema_version": WORLD_STATE_HASH_SCHEMA_VERSION_V3,
        "initial_state": {
